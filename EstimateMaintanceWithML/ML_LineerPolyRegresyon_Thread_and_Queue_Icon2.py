...
ESTIMATE_ERROR_CONSTANT=200 # ürünün tahmini arızalanma sabiti 
# Üründeki titreşimi, bu sabiti aştığı gün cihazın bakım günü olarak kabul edilir.
...
listStr=["High Temp!  ","High Humidity!","High Vibration!","High Current!","High Voltage!"] 
# sıcaklık,nem,vibration,akım,voltage
...
def thread_islem(kuyruk1,kuyruk2,kuyruk3,kuyruk4,kuyruk5):
 ...
 while True:  # döngü oluşturulur belirli sürede bir bu kısır döngü tekrar edilir.
     ...
     time.sleep(2)
     feild_1,feild_2,feild_3,feild_4,feild_5=Thingspeak.read_data_thingspeak() 
    # thinkspeakten 20 adet veri alınır
...
class MplCanvas(FigureCanvasQTAgg):
    ...
    def __init__(self, parent=None, width=4, height=5, dpi=100):
        ...
        fig.subplots_adjust(bottom=0.17,wspace = 0.1,hspace=1.1) 
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setIcon(self):
        icon_path = os.path.abspath("./icons2/iconlogo.ico") 
        #kod dizine icons isimli klasör oluştur ve içine (png to ico) dönüştürdüğün ikonu at.
        #Open folder ile dosya konumunu ekle yoksa debugta çalışır run ettiğinde ikon gelmez.
    ...
    def estimating(self):
        ...
        self.xAldata= x.reshape(len(x),1)
        self.yAldata= y.reshape(len(y),1)

        if len(self.y)>10:
                 print("işlenen data",len(self.y))
        

                 #Polinom Reg.
                 #Not: Polinom regresyon yapmak için önce lineer regresyon uygulanır!!!
                 poli_reg = PolynomialFeatures(degree=1)  #polinom regresyon
                 xPolinom = poli_reg.fit_transform(self.xAldata)

                 lin_reg2= LinearRegression()
                 lin_reg2.fit(xPolinom,self.yAldata) 
                 lin_reg2.predict(xPolinom)
 
                 for i in range(len(self.xAldata)*100):
                  if lin_reg2.predict(poli_reg.fit_transform([[i]]))>ESTIMATE_ERROR_CONSTANT:
                     datenow=datetime.today()
                     Maintanceday=datenow+timedelta(days =i- len(self.xAldata)) 
                     # Elimizde len(self.xAldata) adet data var ve bu dataları i gün sonra eşik değeri aştığı için 
                     # (i-elimizdeki data) yaparak kaç gün sonra eşik değer aşılır onu elde ederiz.
                     print("Tahmini cihaz bakımı ",i- len(self.xAldata)," gün sonra", Maintanceday.date()) 
                     # bu değerden sonra ESTIMATE_ERROR_CONSTANT geçilmiş olur ve cihaz bakım ister.
                     self.line0.setText(str(Maintanceday.date()))
                     break

                 if i==(len(self.xAldata)*100):
                     print("Kestirim yapılamıyor algoritma uygun olmayabilir.")
                     self.line0.setText('ALERT!!!')  
...
def update_plot(self):
        ...
        while not kuyruk3.empty():
            ...
            # if len(self.x)<TAKEN_DATA_SAMPLES: # belirten sabit sayısınca değer almalıyız 
            # çünkü estimate data sayısı değişken olursa bakım tarihi değişkenlik gösteriyor.
            self.x.append(temp)
            temp=temp+1
            self.y.append(x[1])
...
class Thingspeak():
  ...
  def read_data_thingspeak():
        URL='https://api.thingspeak.com/channels/2023940/fields/1.json?api_key='
        URL2='https://api.thingspeak.com/channels/2023940/fields/2.json?api_key='
        URL3='https://api.thingspeak.com/channels/2023940/fields/3.json?api_key='
        URL4='https://api.thingspeak.com/channels/2023940/fields/4.json?api_key='
        URL5='https://api.thingspeak.com/channels/2023940/fields/5.json?api_key=' 
        # Read a Channel Field buradan linki kopyalla kanal no değişir
...
class SicaklikNemData():
    def setRawDataSicaklik(self,_feild_1,kuyruk1):
     for x in _feild_1:
        ...
        _id=_id[:-3]  #"Son 3 karakteri silelim" 
        # fakat datalar dakikada 1 kere gönderilmeli saniyeler siliniyor
...
def main():
    ...
    w = MainWindow()
    th0 = threading.Thread(target=thread_islem,args = (kuyruk1,kuyruk2,kuyruk3,kuyruk4,kuyruk5),daemon=True) 
    th1 = threading.Thread(target=thread_islem2,args = (w,),daemon=True) 
    #Yani özetle main thread sonlandığında daemon thread çalışıyor olsa bile sonlandırılır.
    th0.start()
    th1.start()
